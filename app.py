...
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.environ.get('DATABASE_URL', 'postgres://xcyirhkh:S86J2S4mIgp_HgEQP1yCEVkv6hGIYSww@kashin.db.elephantsql.com/xcyirhkh'))
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            db.session.commit()
            flash(f"{g.user.username}'s profile has been updated!")
...
def like_message(message_id):
    ...
    like = Likes.query.filter(Likes.user_id==g.user.id, Likes.message_id==message_id)
    if len(list(like)) > 0:
        liked_id = [l.id for l in like]
        delete_like = Likes.query.get(liked_id)
        db.session.delete(delete_like)
        db.session.commit()
        return redirect ("/")

    like = Likes(user_id=user_id, message_id=msg.id)
    db.session.add(like)
    db.session.commit()
